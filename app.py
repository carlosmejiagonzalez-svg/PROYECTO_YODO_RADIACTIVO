    ...
    if st.form_submit_button("Sincronizar con Drive"):
        if nombre and cedula:
            if (dosis_actual + dosis) > LIMITE_SEMANAL:
                st.sidebar.error(f"Límite superado. Cupo: {restante}")
            else:
                fecha_str = fecha.strftime("%d/%m/%Y")
                params = {"nombre": nombre, "id": cedula, "entidad": entidad, "fecha": fecha_str, "mci": dosis}
                try:
                    requests.get(SCRIPT_URL, params=params)
                    st.session_state.lista_local.append({"Nombre": nombre, "ID": cedula, "Entidad": entidad, "Fecha_Capsula": fecha_str, "mCI": dosis})
                    st.rerun()
                except:
...
if st.session_state.lista_local:
    ...
    headers = ["Nombre", "ID", "Entidad", "Fecha", "mCI", "Acción"]
    ...
    if st.download_button(
        label="📥 Descargar Reporte y LIMPIAR TODO",
        data=pdf,
        file_name=f"programacion_{datetime.now(colombia_tz).strftime('%d_%m_%Y')}.pdf",
        mime="application/pdf",
        use_container_width=True
    ):
        # LIMPIEZA TOTAL: Local y en Drive
        try:
            requests.post(SCRIPT_URL) # Enviamos señal de limpieza al Apps Script
            st.session_state.lista_local = []
            st.success("Reporte generado y hoja de Drive limpia.")
            st.rerun()
        except:
            st.error("Reporte descargado, pero no se pudo limpiar Drive automáticamente.")
else:
